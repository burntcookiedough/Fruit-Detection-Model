"""Inference on images or webcam with TTA support."""
import os
import sys
...
# ImageNet normalization (must match training pipeline in dataset.py)
...
def run_image_tta(model, img_path, device, anchors):
    """Test-Time Augmentation: run original + flipped, merge predictions."""
    ...
    # --- Horizontal flip ---
    t_flip = TF.hflip(transforms.ToTensor()(img_resized))
    t_flip = TF.normalize(t_flip, IMAGENET_MEAN, IMAGENET_STD).unsqueeze(0).to(device)
    with torch.no_grad():
        cls_pred, box_pred, _ = model(t_flip)
        boxes_f, labels_f, scores_f = decode_predictions(
            cls_pred[0], box_pred[0], anchors, CONF_THRESH, NMS_IOU,
            PRE_NMS_TOPK, MAX_DETECTIONS
        )
    if boxes_f.numel() > 0:
        # Flip boxes back: x1_new = IMG_SIZE - x2_old, x2_new = IMG_SIZE - x1_old
        boxes_f_orig = boxes_f.clone()
        boxes_f[:, 0] = IMG_SIZE - boxes_f_orig[:, 2]
        boxes_f[:, 2] = IMG_SIZE - boxes_f_orig[:, 0]
    ...
    nms_keep = ops.batched_nms(merged_boxes, merged_scores, merged_labels, NMS_IOU)
    nms_keep = nms_keep[:MAX_DETECTIONS]
    boxes = merged_boxes[nms_keep]
    labels = merged_labels[nms_keep]
    scores = merged_scores[nms_keep]
...
def run_image(model, img_path, device, anchors, use_tta=False):
    if use_tta:
        out = run_image_tta(model, img_path, device, anchors)
        out_path = os.path.splitext(img_path)[0] + '_detected.jpg'
        cv2.imwrite(out_path, cv2.cvtColor(out, cv2.COLOR_RGB2BGR))
        print(f"Saved (TTA): {out_path}")
        return
...
def main():
    ...
    if args.mode == 'image':
        if not args.input:
            ...
            return
        run_image(model, args.input, device, anchors, use_tta=args.tta)
    else:
